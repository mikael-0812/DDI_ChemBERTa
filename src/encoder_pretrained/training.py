...
from src.pos_enc.pos_enc import dataset_precomputing
...
def main():
    ...
    if args.pe_type != 'none':
        dataset = MoleculeDataset_Eig_v2('dataset/' + dataset_name, dataset=dataset_name, args=args)
    ...
    if args.block_mask:
        loader = DataLoaderSL_v3(args.mask_rate, args.block_size, dataset, batch_size=args.batch_size, shuffle=True,
                                 num_workers=args.num_workers, pin_memory=True)
    else:
        loader = DataLoaderSL(args.mask_rate, dataset, batch_size=args.batch_size, shuffle=True,
                              num_workers=args.num_workers, pin_memory=True)
    ...
    model = TokenMAE(args.gnn_encoder_layer, args.gnn_token_layer, args.gnn_decoder_layer, args.gnn_emb_dim,
                     args.nonpara_tokenizer, args.gnn_JK, args.gnn_dropout, args.gnn_type,
                     args.d_model, args.trans_encoder_layer, args.trans_decoder_layer, args.nhead, args.dim_feedforward,
                     args.transformer_dropout, args.transformer_activation, args.transformer_norm_input,
                     custom_trans=args.custom_trans, drop_mask_tokens=args.drop_mask_tokens,
                     use_trans_decoder=args.use_trans_decoder, pe_type=args.pe_type, args=args)
