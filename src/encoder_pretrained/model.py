...
from src.pos_enc.encoder import PosEncoder
...
class GINConv(MessagePassing):
    ...
    def __init__(self, emb_dim, out_dim, aggr="add", act_func='relu', **kwargs):
        ...
        self.mlp = torch.nn.Sequential(torch.nn.Linear(emb_dim, 2 * emb_dim), get_activation(act_func),
...
class TokenMAE(torch.nn.Module):
    ...
    def __init__(self, gnn_encoder_layer, gnn_token_layer, gnn_decoder_layer, gnn_emb_dim, nonpara_tokenizer=False,
                 gnn_JK="last", gnn_dropout=0, gnn_type="gin",
...
class GNNDecoder_v3(torch.nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim, gnn_layer=1, drop_ratio=0, gnn_type="gin",
        ...
        if self.trans_layer > 0:
            ...
            self.trans_decoder = TransformerNodeDecoder(d_model, trans_layer, nhead, dim_feedforward,
                                                        transformer_dropout, transformer_activation,
                                                        transformer_norm_input, custom_trans=custom_trans)
    ...
    def forward(self, x, edge_index, edge_attr, masked_tokens, batch):
        ...
        if self.trans_layer > 0:
            ...
            pad_memory, pad_memory_mask = to_dense_batch(memory,
                                                         memory_batch)  # shape = [B, N_max, D], shape = [B, N_max]

            pad_x = pad_x.permute(1, 0, 2)
            pad_memory = pad_memory.permute(1, 0, 2)

            pad_out = self.trans_decoder(pad_x, pad_memory, ~pad_mask,
                                         ~pad_memory_mask)  # discard the cls token; shape = [N_max+1, B, D]
